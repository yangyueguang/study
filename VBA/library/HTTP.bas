...
Function XHttpGET(url As String) As String
    Dim netManager As Object
    Set netManager = CreateObject("Microsoft.XMLHTTP")
'     Dim netManager As New WinHttpRequest
    netManager.Open "GET", url, True
    netManager.send ""
    Do While netManager.readyState <> 4
        DoEvents
    Loop
    If netManager.readyState = 4 Then
        'XHttpGET = StrConv(netManager.responsebody, vbUnicode) '当h.ReadyState=4时说明源码加载完毕
        XHttpGET = StrConv(netManager.ResponseBody, vbUnicode, &H804) '当h.ReadyState=4时说明源码加载完毕  增加乱码解决问题
        Debug.Print (netManager.ResponseBody)
        Debug.Print (XHttpGET)
    End If
End Function
...
Public Function ParseXml(Encoded As String) As Object
    ...
    web_ErrorMsg = "XML is not currently supported (An updated parser is being created that supports Mac and Windows)." & vbNewLine & _
        ...
        vbNewLine & "https://github.com/VBA-tools/VBA-Web/wiki/XML-Support-in-4.0"
...
' WebHelpers.RegisterConverter "csv", "text/csv", "ConvertToCSV", "ParseCSV", Instance:=Converter
