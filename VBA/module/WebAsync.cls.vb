...
Public Property Set Client(Value As WebClient)
    ...
    Else
        ...
        web_ErrorDescription = "The Client for a WebAsync should not be changed as it may affect any currently executing Requests. " & _
            "A new WebAsync should be created for each WebClient."
        WebHelpers.LogError web_ErrorDescription, "WebAsync.Client", vbObjectError + 11050
        Err.Raise vbObjectError + 11050, "WebAsync.Client", web_ErrorDescription
    End If
End Property
...
Public Sub TimedOut()
    ...
    WebHelpers.LogDebug "Timed out", "WebAsync.TimedOut"
    ' Callback
...
Private Sub web_RunCallback(web_Response As WebResponse)
    ...
    End If
    If Me.Callback <> "" Then
        WebHelpers.LogDebug Me.Callback, "WebAsync.RunCallback"
